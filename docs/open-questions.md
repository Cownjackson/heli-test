...
| 10 | How does a pilot look around to find a target? The camera heading is welded to the airframe's, so you can only look where the nose points. **Edge-yaw is now prototyped behind `edge_yaw_enabled`, `F6` to toggle** — fly it and decide. The airframe blocking the shot is unaddressed. | [flight-model.md](flight-model.md#the-edge-yaw-prototype) | flight tuning |
